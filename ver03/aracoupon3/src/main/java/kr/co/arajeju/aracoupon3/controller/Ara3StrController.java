...
@Controller
@RequestMapping(value = "/str3")
public class Ara3StrController {

	private final Log log = LogFactory.getLog(getClass());

	private static final String PATH = "ara3/str";

	@Resource(name = "ara3StrService")
	private Ara3StrService ara3StrService;
	...
	public String indexPost(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			Map<String,Object> map = this.ara3StrService.selectItemInfo(modelMap);
			log.debug("map: " + map);
	...
	public void selectCampList2(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3StrService.selectCampList2(modelMap);
			log.debug("list: " + list);
	...
	public void buyCpnList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			// 소유하고 있는 계좌금액을 얻는다.
			modelMap.putAll(this.ara3StrService.selectStrAcntInfo(modelMap));
		}
		if (Flag.flag) {
			//
			...
			if (cpnSum <= acntBlnc) {
				...
				for (int i=0; i < cpnSiz; i++) {
					int ret = this.ara3StrService.updateBuyCpnSht(modelMap);
					if (Flag.flag) log.debug(">>>>> ret of this.ara3StrService.updateBuyCpnSht is " + ret);
					updCnt += ret;
				}
				...
				int ret = this.ara3StrService.insertBuyCpnSum(modelMap);
				if (Flag.flag) log.debug(">>>>> ret of this.ara3StrService.insertBuyCpnSum is " + ret);
				//
	...
	public void selectStateCpnList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3StrService.selectStateCpnList(modelMap);
			log.debug("list: " + list);
			modelMap.addAttribute("list", list);
			modelMap.addAttribute("retCode", "0000");
			modelMap.addAttribute("retMsg", "성공적으로 자료를 얻었습니다.");
	...
	public String giveCpnListPage(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> lst = this.ara3StrService.selectTrmList(modelMap);
			log.debug("lst: " + lst);
	...
	public void selectGiveCpnList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3StrService.selectGiveCpnList(modelMap);
			log.debug("list: " + list);
			modelMap.addAttribute("list", list);
			modelMap.addAttribute("retCode", "0000");
			modelMap.addAttribute("retMsg", "성공적으로 자료를 얻었습니다.");
	...
	public void sendAraKeyToUsr(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			...
			map = this.ara3StrService.selectUsrInfo(modelMap);
			if (map == null) {
				modelMap.addAttribute("retCode", "9999");
				...
				if (Flag.flag) {
					...
					this.ara3StrService.updateAllCnntByCnntid(modelMap);
				}
				if (Flag.flag) {
					// arakey를 얻는다.
				...
				if (Flag.flag) {
					...
					this.ara3StrService.insertCnntAraKey(modelMap);
				}
				if (Flag.flag) {
					// SMS 전송
	...
	public void giveCpnList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			Map<String,Object> map = this.ara3StrService.selectCnntArakeyInfo(modelMap);
			if (map == null && String.valueOf(modelMap.get("arakeyYn")) == "Y") {
				...
				int updCnt = 0;
				for (int i=0; i < cpnSiz; i++) {
					int ret = this.ara3StrService.updateCpnNo(modelMap);
					if (Flag.flag) log.debug(">>>>> ret of this.ara3StrService.updateBuyCpnSht is " + ret);
					updCnt += ret;
				}
				...
				this.ara3StrService.updateCnntByCnntid(modelMap);
			}
		}
		if (Flag.flag) log.debug(">>>>> modelMap: " + new GsonBuilder().setPrettyPrinting().create().toJson(modelMap));
		jsonView.render(modelMap, request, response);
	...
	public void selectPaymentCpnList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3StrService.selectPaymentCpnList(modelMap);
			modelMap.addAttribute("list", list);
		}
		if (Flag.flag) {
			modelMap.addAttribute("retCode", "0000");
	...
	public void sendAraKeyToCtr(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			// id를 확인한다.
			Map<String,Object> map = null;
			map = this.ara3StrService.selectCtrInfo(modelMap);
			if (map == null) {
				modelMap.addAttribute("retCode", "9999");
				modelMap.addAttribute("retMsg", String.format("요청하신 아이디가 존재하지 않습니다."));
			} else {
				...
				if (Flag.flag) {
					// 이미 진행중인 아라키는 사용하지 못하도록 한다.
					this.ara3StrService.updateAllCnntByCnntidOnCtr(modelMap);
				}
				if (Flag.flag) {
					// arakey를 얻는다.
					String arakey = GenerateAraKey.getAraKey();
				...
				if (Flag.flag) {
					// arakey를 ARA_CNNT 테이블에 넣는다.
					this.ara3StrService.insertCnntAraKeyOnCtr(modelMap);
				}
				if (Flag.flag) {
					// SMS 전송
					map = this.ara3IbService.selectLastIbTkn(null);
	...
	public void calculateCpnNoList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (!Flag.flag) {
			// use the arakey
			Map<String,Object> map = this.ara3StrService.selectCnntArakeyInfoOnCtr(modelMap);
			if (map == null) {
				// (userid, arakey) 존재하지 않으면 인증 실패
				if (Flag.flag) {
					modelMap.addAttribute("retCode", "9999");
				...
				// (userid, arakey) 존재하면 정보를 리턴하고 자료를 update 한다.
				this.ara3StrService.updateCpnNoListOnCtr(modelMap);
				if (Flag.flag) {
					modelMap.addAttribute("retCode", "0000");
					modelMap.addAttribute("retMsg", String.format("정상적으로 처리되었습니다."));
				}
				modelMap.addAttribute("map", map);
				this.ara3StrService.updateCnntByCnntidOnCtr(modelMap);
			}
		}
		if (Flag.flag) {
			// not use the arakey
	...
	public void selectInOutAcntList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3StrService.selectAcntInOutList(modelMap);
			log.debug("list: " + list);
			modelMap.addAttribute("list", list);
		}
		if (Flag.flag) {
