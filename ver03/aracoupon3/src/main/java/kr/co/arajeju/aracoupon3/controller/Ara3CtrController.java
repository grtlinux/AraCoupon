...
@Controller
@RequestMapping(value = "/ctr3")
public class Ara3CtrController {

	private final Log log = LogFactory.getLog(getClass());

	private static final String PATH = "ara3/ctr";

	@Resource(name = "ara3CtrService")
	private Ara3CtrService ara3CtrService;
	...
	public String indexPost(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			Map<String,Object> map = this.ara3CtrService.selectItemInfo(modelMap);
			log.debug("map: " + map);
	...
	public String createCampFormPage(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			Map<String,Object> map = this.ara3CtrService.selectCampPeriod(modelMap);
			log.debug("period: " + map);
			modelMap.addAttribute("period", map);
		}
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3CtrService.selectAraCpnTyp(modelMap);
	...
	public void createCampInfo(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			// get CAMP_ID
			Map<String,Object> map = this.ara3CtrService.selectSeqCampKey(modelMap);
			if (map == null) {
				...
				this.ara3CtrService.insertCampInfo(modelMap);
	...
	public void selectCampList1(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3CtrService.selectCampList1(modelMap);
			log.debug("list: " + list);
	...
	public void insertApprReq(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			...
			for (int i=0; i < arrCampNo.length; i++) {
				...
				// 해당 캠페인의 쿠폰을 모두 삭제한다.
				this.ara3CtrService.deleteCampCpnSht(map);
				// 해당 캠페인정보를 얻는다.
				map.putAll(this.ara3CtrService.selectCampNoInfo(map));
				...
				for (int seq=1; seq <= cnt; seq++) {
					...
					map.put("cpnNo", cpnNo);
					// 쿠폰 한장을 생성한다.
					this.ara3CtrService.insertCampCpnSht(map);
				}
				// 해당 캠페인에 대한 쿠폰페키지를 발행했으면 캠페인정보를 변경한다.
				this.ara3CtrService.updateCampNoInfo(map);
			}
			// 정상 쿠폰발행
			modelMap.addAttribute("retCode", "0000");
	...
	public void selectCampList2(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3CtrService.selectCampList2(modelMap);
			log.debug("list: " + list);
			modelMap.addAttribute("list", list);
		}
		if (Flag.flag) log.debug(">>>>> modelMap: " + new GsonBuilder().setPrettyPrinting().create().toJson(modelMap));
	...
	public void selectPaymentCpnList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3CtrService.selectPaymentCpnList(modelMap);
			log.debug("list: " + list);
			modelMap.addAttribute("list", list);
		}
		if (Flag.flag) log.debug(">>>>> modelMap: " + new GsonBuilder().setPrettyPrinting().create().toJson(modelMap));
	...
	public void calculateCpnNoList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			int ret = 0;
			// ara3_cpn.cpn_phs: 5 -> 0
			modelMap.addAttribute("phsFrm", "5");
			modelMap.addAttribute("phsTo", "0");
			ret = this.ara3CtrService.updateCpnNosPhs(modelMap);
			if (Flag.flag) log.debug(">>>>> ret of this.ara3CtrService.updateCpnNosPhs is " + ret);
			
			// group by str -> sum, insert into ara3_acnthist
			ret = this.ara3CtrService.insertAcntDpst(modelMap);
			if (Flag.flag) log.debug(">>>>> ret of this.ara3CtrService.insertAcntDpst is " + ret);
			
			// ara3_cpn.cpn_phs: P -> 6
	...
	public void selectCompleteCalcList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3CtrService.selectCompleteCalcList(modelMap);
			log.debug("list: " + list);
			modelMap.addAttribute("list", list);
		}
		if (Flag.flag) {
	...
	public void selectNotUsedCpnList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3CtrService.selectNotUsedCpnList(modelMap);
			log.debug("list: " + list);
			modelMap.addAttribute("list", list);
		}
		if (Flag.flag) {
	...
	public void selectSettlementDiscardList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3CtrService.selectSettlementDiscardList(modelMap);
			log.debug("list: " + list);
			modelMap.addAttribute("list", list);
		}
		if (Flag.flag) {
	...
	public void selectStrAcntList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3CtrService.selectStrAcntList(modelMap);
			log.debug("list: " + list);
			modelMap.addAttribute("list", list);
		}
		if (Flag.flag) {
	...
	public void createStrAcnt(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			int ret = this.ara3CtrService.insertStrAcnt(modelMap);
			log.debug("ret: " + ret);
	...
	public void selectAcntList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3CtrService.selectAcntList(modelMap);
			log.debug("list: " + list);
			modelMap.addAttribute("list", list);
		}
		if (Flag.flag) {
	...
	public String selectAcntInOutListPage(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> listStr = this.ara3CtrService.selectStrList(modelMap);
	...
	public void selectInOutAcntList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3CtrService.selectAcntInOutList(modelMap);
			log.debug("list: " + list);
			modelMap.addAttribute("list", list);
		}
		if (Flag.flag) {
	...
	public void insertStrDpstProc(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			int ret = this.ara3CtrService.insertStrDpstProc(modelMap);
			log.debug("ret: " + ret);
			modelMap.addAttribute("ret", ret);
		}
		if (Flag.flag) {
	...
	public void insertStrWthdProc(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			int ret = this.ara3CtrService.insertStrWthdProc(modelMap);
			log.debug("ret: " + ret);
			modelMap.addAttribute("ret", ret);
		}
		if (Flag.flag) {
	...
	public void deleteTablesForOpen(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			int ret = this.ara3CtrService.deleteTableAcnt(modelMap);
			log.debug("deleteTableAcnt.ret: " + ret);
			modelMap.addAttribute("ret", ret);
		}
		if (Flag.flag) {
			int ret = this.ara3CtrService.deleteTableAcnthist(modelMap);
			log.debug("deleteTableAcnthist.ret: " + ret);
			modelMap.addAttribute("ret", ret);
		}
		if (Flag.flag) {
			int ret = this.ara3CtrService.deleteTableCamp(modelMap);
			log.debug("deleteTableCamp.ret: " + ret);
			modelMap.addAttribute("ret", ret);
		}
		if (Flag.flag) {
			int ret = this.ara3CtrService.deleteTableCpn(modelMap);
			log.debug("deleteTableCpn.ret: " + ret);
			modelMap.addAttribute("ret", ret);
		}
		if (Flag.flag) {
			int ret = this.ara3CtrService.deleteTableCnnt(modelMap);
			log.debug("deleteTableCnnt.ret: " + ret);
			modelMap.addAttribute("ret", ret);
		}
		if (Flag.flag) {
			int ret = this.ara3CtrService.deleteTableLogin(modelMap);
	...
	public void createAllLoginInfo(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3CtrService.selectAllLoginInfo(modelMap);
			...
			for (Map<String,Object> map : list) {
				...
				int ret = this.ara3CtrService.insertLoginInfo(map);
				log.debug("deleteTableLogin.ret: " + ret);
			}
		}
		if (Flag.flag) {
	...
	public void selectLikeLoginSrch(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3CtrService.selectLikeLoginSrch(modelMap);
			log.debug("list: " + list);
			modelMap.addAttribute("list", list);
		}
		if (Flag.flag) {
	...
	public void selectStrInfoList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3CtrService.selectStrInfoList(modelMap);
			log.debug("list: " + list);
			modelMap.addAttribute("list", list);
		}
		if (Flag.flag) {
	...
	public void selectUsrInfoList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			List<Map<String,Object>> list = this.ara3CtrService.selectUsrInfoList(modelMap);
			log.debug("list: " + list);
			modelMap.addAttribute("list", list);
		}
		if (Flag.flag) {
