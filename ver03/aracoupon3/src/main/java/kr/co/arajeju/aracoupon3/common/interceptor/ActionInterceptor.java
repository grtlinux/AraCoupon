...
import kr.co.arajeju.aracoupon3.vo.ContextParameter;
...
public class ActionInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if (log.isDebugEnabled()) {
			...
			log.debug("KANG-20190811" + contextParameter);
		}
		...
		if (Flag.flag) {
			// 업무시간에 대한 처리를 한다.
			String araNowTime = Flag.getDateTime(contextParameter.getAraSalesType());
			request.setAttribute("araNowTime", araNowTime);
			
			if (araNowTime.compareTo(contextParameter.getAraSalesOpenTime()) < 0
					|| contextParameter.getAraSalesCloseTime().compareTo(araNowTime) < 0) {
				// 업무시간이 아니면 접속페이지로 이동한다.
				if (Flag.flag) log.debug(String.format("KANG-20190807 redirect:/ara2/index.do   because of not sales time: %s (%s~%s)"
						, araNowTime
						, contextParameter.getAraSalesOpenTime()
						, contextParameter.getAraSalesCloseTime()));
