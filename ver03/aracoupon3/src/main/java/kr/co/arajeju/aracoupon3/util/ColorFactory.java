...
public class ColorFactory {
	...
	public Color getColor(String colorSpec) {
		String normalSpec = colorSpec.toLowerCase().replace("\\s", "");
		synchronized (this) {
			Color color = (Color) this.colorMap.get(normalSpec);                       // yellow, red
			if (color == null) {
				if (normalSpec.startsWith(RGB_START)) {
					// CssParser produces this format.                                 // rgb(255, 255, 125)
				...
				} else if (normalSpec.startsWith("#")) {                                 // #ffab25
