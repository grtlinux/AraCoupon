package kr.co.arajeju.aracoupon2.controller;
...
import kr.co.arajeju.aracoupon2.service.Ara2IbService;
...
import kr.co.arajeju.aracoupon2.util.GenerateAraKey;
import kr.co.arajeju.aracoupon2.util.IbMsg;
...
public class Ara2UsrController {
	...
	public void sendAraKeyToStr(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			...
			map = this.ara2UsrService.selectStrInfo(modelMap);
			if (map == null) {
				...
				if (Flag.flag) {
					...
					this.ara2UsrService.updateAllCnntByCnntid(modelMap);
				}
				if (Flag.flag) {
					// arakey를 얻는다.
				...
				if (Flag.flag) {
					...
					this.ara2UsrService.insertCnntAraKey(modelMap);
				}
				if (Flag.flag) {
					// SMS 전송
	...
	public void useCpnNoList(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		if (Flag.flag) {
			Map<String,Object> map = this.ara2UsrService.selectCnntArakeyInfo(modelMap);
			if (map == null && String.valueOf(modelMap.get("arakeyYn")) == "Y") {
				...
				this.ara2UsrService.updateCpnNoList(modelMap);
				if (Flag.flag) {
					modelMap.addAttribute("retCode", "0000");
					modelMap.addAttribute("retMsg", String.format("정상적으로 처리되었습니다."));
				}
				modelMap.addAttribute("map", map);
				this.ara2UsrService.updateCnntByCnntid(modelMap);
			}
		}
		if (Flag.flag) log.debug(">>>>> modelMap: " + new GsonBuilder().setPrettyPrinting().create().toJson(modelMap));
		jsonView.render(modelMap, request, response);
