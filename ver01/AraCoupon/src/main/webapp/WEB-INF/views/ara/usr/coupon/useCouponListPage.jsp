	...
	<div class="container-fluid">
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h3 class="panel-title"><span class="glyphicon glyphicon-tags"></span>&nbsp;&nbsp;&nbsp;사용 가능한 쿠폰목록</h3>
			</div>
			<div class="panel-body">
				<div id="sysbtn1" class="text-right show" style="margin-bottom:10px;">
					...
					<button type="button" class="btn btn-danger btn-sm" onclick="fn_choiceCpnNos();"> 쿠폰사용 </button>
				</div>
	...
	<div class="row">
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-footer text-right">
						<button type="button" class="btn btn-info btn-sm" onclick="fn_cancelModal1();"> 취소 </button>
					</div>
				</div>
			</div>
		</div>
		<div class="modal" id="modal2" tabindex="-1">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						쿠폰을 사용<button class="close" data-dismiss="modal">&times;</button>
					</div>
					<div class="modal-body text-center">
						<div class="table-responsive">
						...
						<div class="text-center">
							<table id="tempTable0" class="table table-hover table-condensed">
								<tbody>
									<tr>
										<td>사용 가게ID</td>
										<td><input id='strid' type="text" value=''></td>
									...
									<tr>
										...
										<td class='text-right'><button type='button' class='btn btn-danger btn-sm' onclick="fn_useCpnList();">쿠폰사용</button>
									</tr>
								</tbody>
							</table>
						</div>
					...
					<div class="modal-footer text-right show">
						<button type="button" class="btn btn-info btn-sm" onclick="fn_cancelModal2();"> 취소 </button>
						<button type="button" class="btn btn-danger btn-sm hide" onclick="fn_useCpnList();"> 쿠폰사용 </button>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript">
	...
	function selectUseCpnList(){
		jQuery.ajax({
			url           : "${staticPATH}/usr/coupon/selectUseCpnList.do",
			dataType      : "JSON",
			scriptCharset : "UTF-8",
			type          : "POST",
	...
	function fn_cancelModal1() {
		$('#modal1').modal('toggle');
	}
	function fn_cancelModal2() {
		$('#modal2').modal('toggle');
	}
	function fn_useCpnList() {
		// validate
		if (true) {
			if (isEmpty($('#modal2 #strid').val())) {
				$('#modal2 #strid').focus();
				return false;
			}
		...
		// transfer
		if (true) {
			$("#tempForm #_strid").val($("#modal2 #strid").val());
		...
		// ajax
		jQuery.ajax({
			url           : "${staticPATH}/usr/coupon/useCpnList.do",
			dataType      : "JSON",
			scriptCharset : "UTF-8",
			type          : "POST",
			data          : $('#tempForm').serialize(),
